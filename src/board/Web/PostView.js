...
const PostView = () => {
  ...
  useEffect(() => {
    const fetchPostData = async () => {
      try {
        const postRef = doc(db, 'posts3', idx);
  ...
  const handleCommentSubmit = async (postId) => {
    ...
    try {
      // 댓글 추가
      const commentRef = await addDoc(collection(db, 'comments3'), commentData);
  ...
  const handleCommentDelete = async (commentId) => {
    try {
      ...
      await deleteDoc(doc(db, 'comments3', commentId));
  ...
  return (
    <>
      <div className="post-view-wrapper">
        ...
        <button className="post-view-go-list-btn" onClick={() => navigate('/Web')}>
          목록으로 돌아가기
        </button>
      </div>
    </>
