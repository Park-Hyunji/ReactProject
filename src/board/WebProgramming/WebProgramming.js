...
const WebProgramming = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const postCollection = collection(db, 'posts5');
  ...
  const handleDeleteConfirm = async () => {
    try {
      const postRef = doc(db, 'posts5', deleteTargetId);
  ...
  return (
    <>
      ...
      <p style={{fontFamily: 'bori', fontWeight: 'bold', fontSize: '30px', color: 'skyblue' , marginLeft : "680px"}}>웹프로그래밍 게시판</p><img src={edit} alt={edit} style={{width: "30px", marginRight:"180px" }} onClick={() => navigate('./writer')} /> </div>
      ...
      <CommonTable headersName={['글번호', '제목', '작성일', '수정','삭제']}  >
        {currentPosts.map((item, index) => (
          <CommonTableRow key={index + indexOfFirstPost} >
            ...
            <CommonTableColumn>
              <Link to={`/WebProgramming/${index + indexOfFirstPost}`} state={{ data: currentPosts }}  style={{ fontWeight: 'bold' ,fontFamily: 'bori', fontSize: '20px', color: 'black', textDecoration: 'none' }} >
                {item.title}
              </Link>
            </CommonTableColumn>
