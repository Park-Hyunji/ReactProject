...
const Cloud = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const postCollection = collection(db, 'posts8');
  ...
  const handleDeleteConfirm = async () => {
    try {
      const postRef = doc(db, 'posts8', deleteTargetId);
  ...
  return (
    <>
      ...
      <p style={{fontFamily: 'bori', fontWeight: 'bold', fontSize: '30px', color: 'skyblue' , marginLeft : "680px"}}>클라우드컴퓨팅 게시판</p><img src={edit} alt={edit} style={{width: "30px", marginRight:"180px" }} onClick={() => navigate('./writer')} /> </div>
      ...
      <CommonTable headersName={['글번호', '제목', '작성일', '수정','삭제']}  >
        {currentPosts.map((item, index) => (
          <CommonTableRow key={index + indexOfFirstPost} >
            ...
            <CommonTableColumn>
              <Link to={`/Cloud/${index + indexOfFirstPost}`} state={{ data: currentPosts }}  style={{ fontWeight: 'bold' ,fontFamily: 'bori', fontSize: '20px', color: 'black', textDecoration: 'none' }} >
                {item.title}
              </Link>
            </CommonTableColumn>
